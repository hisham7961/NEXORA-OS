...
export function WebhookManager({ webhooks, knownEvents }: { webhooks: WebhookRow[]; knownEvents: string[] }) {
  ...
  const create = () => {
    ...
    if (evts.length === 0) { toast({ kind: "error", title: t("sec.addOneEvent") }); return; }
    start(async () => {
  ...
  const remove = (id: string, nm: string) => {
    if (!confirm(t("sec.deleteWebhookConfirm", { name: nm }))) return;
    start(async () => { const r = await deleteWebhookAction(id); if (r.ok) { toast({ kind: "success", title: t("sec.webhookDeleted") }); router.refresh(); } else toast({ kind: "error", title: r.error }); });
  };
    ...
    <div className="space-y-3">
      {fresh && (
        <div className="rounded-lg border border-accent bg-accent-soft/40 p-3">
          <div className="mb-1 flex items-center gap-1.5 text-[12px] font-semibold text-accent"><Webhook className="h-3.5 w-3.5" /> {t("sec.signingSecretFor", { name: fresh.name })}</div>
      ...
      <div className="rounded-lg border border-line p-3">
        ...
        <div className="mt-2"><Button variant="primary" size="sm" disabled={pending || !name.trim() || !url.trim()} onClick={create}><Plus className="h-3.5 w-3.5" /> {t("sec.addWebhook")}</Button></div>
      </div>

      <ul className="divide-y divide-line rounded-lg border border-line">
        {webhooks.length === 0 ? (
          <li className="px-3 py-6 text-center text-[13px] text-ink-3">{t("sec.noWebhooks")}</li>
        ) : webhooks.map((w) => (
          <li key={w.id} className="flex items-center justify-between gap-3 px-3 py-2.5">
            <div className="min-w-0">
              <div className="flex items-center gap-2 text-[13px] text-ink">
                <span className="truncate font-medium">{w.name}</span>
                {!w.active && <span className="rounded bg-surface-2 px-1.5 py-0.5 text-[10px] font-semibold uppercase text-ink-3">{t("sec.disabledBadge")}</span>}
              </div>
              <div className="truncate font-mono text-[11px] text-ink-3">{w.url}</div>
              <div className="text-[11px] text-ink-3">{t("sec.eventsPrefix", { events: w.events.join(", ") })}</div>
            </div>
            <div className="flex shrink-0 items-center gap-1">
              <Button size="sm" variant="ghost" disabled={pending} onClick={() => toggle(w.id, !w.active)}><Power className="h-3.5 w-3.5" /> {w.active ? t("sec.disable") : t("sec.enable")}</Button>
