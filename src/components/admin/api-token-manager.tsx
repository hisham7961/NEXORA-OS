...
export function ApiTokenManager({ tokens }: { tokens: TokenRow[] }) {
  ...
  const create = () => {
    ...
    start(async () => {
      ...
      else toast({ kind: "error", title: r.ok ? "No token returned" : r.error });
    });
  };

  const revoke = (id: string, tname: string) => {
    if (!confirm(`Revoke token "${tname}"? Any client using it will stop working immediately.`)) return;
    start(async () => {
      const r = await revokeApiTokenAction(id);
      if (r.ok) { toast({ kind: "success", title: "Token revoked" }); router.refresh(); }
    ...
    <div className="space-y-3">
      {fresh && (
        <div className="rounded-lg border border-accent bg-accent-soft/40 p-3">
          <div className="mb-1 flex items-center gap-1.5 text-[12px] font-semibold text-accent"><KeyRound className="h-3.5 w-3.5" /> New token “{fresh.name}” — copy it now, it won’t be shown again</div>
      ...
      <div className="rounded-lg border border-line p-3">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto_auto_auto] sm:items-end">
          ...
          <div>
            <Label htmlFor="tok-exp">Expires (days)</Label>
            <Input id="tok-exp" type="number" min={1} max={365} value={expiresInDays} onChange={(e) => setExpiresInDays(e.target.value)} placeholder="never" className="w-28" />
          </div>
          ...
          <Button variant="primary" disabled={pending || !name.trim()} onClick={create}><Plus className="h-3.5 w-3.5" /> Create</Button>
        </div>
      </div>

      <ul className="divide-y divide-line rounded-lg border border-line">
        {tokens.length === 0 ? (
          <li className="px-3 py-6 text-center text-[13px] text-ink-3">No API tokens yet. Create one for an integration or the mobile app.</li>
        ) : tokens.map((t) => (
          <li key={t.id} className="flex items-center justify-between gap-3 px-3 py-2.5">
            <div className="min-w-0">
              <div className="flex items-center gap-2 text-[13px] text-ink">
                <span className="truncate font-medium">{t.name}</span>
                {t.readOnly && <span className="rounded bg-surface-2 px-1.5 py-0.5 text-[10px] font-semibold uppercase text-ink-2">read-only</span>}
                {t.status !== "active" && <span className="rounded bg-surface-2 px-1.5 py-0.5 text-[10px] font-semibold uppercase text-critical">{t.status}</span>}
              </div>
              <div className="font-mono text-[11px] text-ink-3">{t.prefix}… · {t.lastUsedAt ? `last used ${new Date(t.lastUsedAt).toLocaleDateString()}` : "never used"}{t.expiresAt ? ` · expires ${new Date(t.expiresAt).toLocaleDateString()}` : ""}</div>
            </div>
            {t.status === "active" && (
              <Button size="sm" variant="ghost" disabled={pending} onClick={() => revoke(t.id, t.name)}><Trash2 className="h-3.5 w-3.5" /> Revoke</Button>
            )}
          </li>
        ))}
      </ul>
