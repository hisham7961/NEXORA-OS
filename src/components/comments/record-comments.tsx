...
export function RecordComments({ entityType, entityId, currentUserId }: { entityType: string; entityId: string; currentUserId: string }) {
  ...
  const CommentRow = ({ c, reply }: { c: Comment; reply?: boolean }) => (
    <div className={`py-2 ${reply ? "ms-6 border-s border-line ps-3" : ""}`}>
      <div className="flex items-center justify-between gap-2 text-[12px]">
        ...
        <span className="text-ink-3">{new Date(c.createdAt).toLocaleString()}{c.editedAt ? ` · ${t("rc.edited")}` : ""}</span>
      </div>
      {editing === c.id ? (
        <div className="mt-1 space-y-1">
          <Textarea value={editBody} onChange={(e) => setEditBody(e.target.value)} rows={2} />
          <div className="flex gap-2"><Button variant="primary" size="sm" disabled={pending} onClick={() => saveEdit(c.id)}>{t("rc.save")}</Button><Button variant="ghost" size="sm" onClick={() => setEditing(null)}>{t("rc.cancel")}</Button></div>
        </div>
      ) : (
      ...
      {editing !== c.id && (
        <div className="mt-1 flex items-center gap-3 text-[11px] text-ink-3">
          {!reply && <button className="inline-flex items-center gap-1 hover:text-ink" onClick={() => { setReplyTo(c.id); }}><CornerDownRight className="h-3 w-3" /> {t("rc.reply")}</button>}
          {c.authorId === currentUserId && <button className="inline-flex items-center gap-1 hover:text-ink" onClick={() => { setEditing(c.id); setEditBody(c.body); }}><Pencil className="h-3 w-3" /> {t("rc.edit")}</button>}
          {c.authorId === currentUserId && <button className="inline-flex items-center gap-1 hover:text-critical" onClick={() => remove(c.id)}><Trash2 className="h-3 w-3" /> {t("rc.delete")}</button>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      <div className="mb-3 flex items-center gap-2 text-[13px] font-medium text-ink"><MessageSquare className="h-4 w-4 text-ink-3" /> {t("rc.comments")} {comments ? `(${comments.length})` : ""}</div>
      <div className="divide-y divide-line">
        {comments === null ? <p className="py-2 text-[13px] text-ink-3">{t("rc.loading")}</p>
          : roots.length === 0 ? <p className="py-2 text-[13px] text-ink-3">{t("rc.noComments")}</p>
          : roots.map((c) => <div key={c.id}>{<CommentRow c={c} />}{repliesOf(c.id).map((r) => <CommentRow key={r.id} c={r} reply />)}</div>)}
      </div>
      <div className="mt-3 space-y-2">
        {replyTo && <div className="text-[11px] text-ink-3">{t("rc.replying")} <button className="text-accent hover:underline" onClick={() => setReplyTo(null)}>{t("rc.cancelLower")}</button></div>}
        <Textarea value={body} onChange={(e) => setBody(e.target.value)} rows={2} placeholder={t("rc.placeholder")} />
        <div className="flex justify-end"><Button variant="primary" size="sm" disabled={pending || !body.trim()} onClick={post}>{t("rc.comment")}</Button></div>
      </div>
    </div>
  );
}
