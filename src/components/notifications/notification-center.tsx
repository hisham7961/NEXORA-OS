    ...
    <>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <div className="flex rounded-md border border-line p-0.5">
          <button onClick={() => setParam("unread", null)} className={`rounded px-2.5 py-1 text-[12px] ${!onlyUnread ? "bg-accent-soft text-accent" : "text-ink-2"}`}>All</button>
          <button onClick={() => setParam("unread", "1")} className={`rounded px-2.5 py-1 text-[12px] ${onlyUnread ? "bg-accent-soft text-accent" : "text-ink-2"}`}>Unread {unread > 0 && `(${unread})`}</button>
        </div>
        ...
        <div className="ms-auto flex gap-2">
          <Button variant="ghost" size="sm" onClick={() => setShowPrefs((v) => !v)}><SlidersHorizontal className="h-3.5 w-3.5" /> Preferences</Button>
          <Button variant="secondary" size="sm" disabled={pending || unread === 0} onClick={() => run(() => markAllReadAction())}><CheckCheck className="h-3.5 w-3.5" /> Mark all read</Button>
        </div>
      </div>

      {showPrefs && (
        <div className="mb-4 rounded-lg border border-line bg-surface-2/40 p-3">
          <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-ink-3">In-app notification preferences</div>
      ...
      {groups.length === 0 ? (
        <div className="rounded-lg border border-line py-12 text-center">
          ...
          <p className="mt-1 text-xs text-ink-3">New notifications appear here, grouped to avoid spam.</p>
        </div>
      ) : (
          ...
          {groups.map((g) => {
            const Body = (
              <div className="flex items-start gap-3 px-4 py-3">
                ...
                <span className="whitespace-nowrap text-[11px] text-ink-3">{timeAgo(g.createdAt)}</span>
              </div>
            );
            return (
              <li key={g.key} className={`group relative ${g.unread ? "bg-accent-soft/20" : ""}`}>
                <div className="flex items-center">
                  ...
                  <div className="flex shrink-0 items-center gap-1 pe-3 opacity-0 group-hover:opacity-100">
                    {g.unread && <button title="Mark read" disabled={pending} onClick={() => run(() => markReadAction(g.ids, true))} className="p-1.5 text-ink-3 hover:text-accent"><Check className="h-3.5 w-3.5" /></button>}
                    <button title="Archive" disabled={pending} onClick={() => run(() => archiveNotificationsAction(g.ids))} className="p-1.5 text-ink-3 hover:text-critical"><Archive className="h-3.5 w-3.5" /></button>
                  </div>
                </div>
              </li>
            );
