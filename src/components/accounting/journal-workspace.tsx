...
export function JournalWorkspace({ companyId, baseCurrency, accounts, journals }: { companyId: string; baseCurrency: string; accounts: AccountOpt[]; journals: { code: string; name: string }[] }) {
  ...
  const post = () => start(async () => {
    ...
    if (res.ok) { toast({ kind: "success", title: `Posted ${res.data?.journalNumber ?? ""}` }); setLines([emptyLine(), emptyLine()]); setMemo(""); setReference(""); router.refresh(); }
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <label className="block"><span className="mb-1 block text-[12px] text-ink-2">Date</span><Input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></label>
        <label className="block"><span className="mb-1 block text-[12px] text-ink-2">Journal</span><Select value={journalCode} onChange={(e) => setJournalCode(e.target.value)}>{journals.map((j) => <option key={j.code} value={j.code}>{j.code} — {j.name}</option>)}</Select></label>
        <label className="block"><span className="mb-1 block text-[12px] text-ink-2">Reference</span><Input value={reference} onChange={(e) => setReference(e.target.value)} placeholder="optional" /></label>
        <label className="block"><span className="mb-1 block text-[12px] text-ink-2">Memo</span><Input value={memo} onChange={(e) => setMemo(e.target.value)} placeholder="optional" /></label>
      </div>
      ...
      <div className="overflow-x-auto rounded-md border border-line">
        <table className="w-full text-[13px]">
          <thead className="bg-surface-2 text-[11px] uppercase tracking-wide text-ink-3">
            <tr>
              <th className="px-2 py-1.5 text-start font-medium">Account</th>
              <th className="px-2 py-1.5 text-start font-medium">Description</th>
              <th className="px-2 py-1.5 text-end font-medium">Debit</th>
              <th className="px-2 py-1.5 text-end font-medium">Credit</th>
              <th className="w-8"></th>
            </tr>
          </thead>
          <tbody className="divide-y divide-line">
            {lines.map((l, i) => (
              <tr key={i}>
                <td className="px-2 py-1"><Select value={l.accountId} onChange={(e) => setLine(i, { accountId: e.target.value })} className="h-8 min-w-[180px]"><option value="">Select…</option>{postable.map((a) => <option key={a.id} value={a.id}>{a.code} — {a.name}</option>)}</Select></td>
          ...
          <tfoot className="bg-surface-2 text-[13px] font-medium">
            <tr>
              <td className="px-2 py-1.5" colSpan={2}><button className="inline-flex items-center gap-1 text-[12px] text-accent hover:underline" onClick={() => setLines((ls) => [...ls, emptyLine()])}><Plus className="h-3.5 w-3.5" /> Add line</button></td>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          {balanced ? (
            <Badge category="success"><Check className="h-3 w-3" /> Balanced</Badge>
          ) : (
            <span className="inline-flex items-center gap-1.5 text-[13px] text-warning"><AlertTriangle className="h-4 w-4" /> Difference: <span className="tabular">{totals.diff.toFixed(3)}</span> {baseCurrency}</span>
          )}
        </div>
        <Button variant="primary" size="sm" disabled={pending || !balanced || filled.length < 2} onClick={post}>{pending ? "Posting…" : "Post entry"}</Button>
      </div>
    </div>
  );
}
