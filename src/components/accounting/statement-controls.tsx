    ...
    <>
      ...
      <Drawer open={open} onClose={() => setOpen(false)} title={t("fin.importBankStatement")} description={t("fin.importStatementDesc")} width="640px">
        <div className="space-y-3">
          ...
          <label className="block"><span className="mb-1 block text-[12px] text-ink-2">{t("fin.orPasteCsv")}</span><Textarea value={csvText} onChange={(e) => { setCsvText(e.target.value); setPreview(null); }} rows={4} placeholder="date,description,reference,amount,balance&#10;2026-01-05,Customer receipt,TT-1,1000,5000" className="font-mono text-[12px]" /></label>
          {headers.length > 0 && (
