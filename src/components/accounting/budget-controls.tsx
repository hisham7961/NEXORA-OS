...
const MONTHS = ["monthAll","month.jan","month.feb","month.mar","month.apr","month.may","month.jun","month.jul","month.aug","month.sep","month.oct","month.nov","month.dec"];
...
export function BudgetComposer({ companyId, accounts, fiscalYears, budget }: { companyId: string; accounts: AccountOption[]; fiscalYears: FyOption[]; budget?: { id: string; name: string; fiscalYearId: string | null; currency: string; periodicity: string; lines: { accountId: string; amount: string; periodMonth: number | null }[] } }) {
  ...
  const submit = () => start(async () => {
    ...
    if (r.ok) { toast({ kind: "success", title: budget ? t("acct.budgetSaved") : t("acct.budgetCreated") }); const id = budget?.id ?? (r as { data?: { id: string } }).data?.id; router.push(id ? `/accounting/budgets/${id}` : "/accounting/budgets"); } else toast({ kind: "error", title: r.error });
  });

  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Field label={t("acct.budgetName")}><Input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("acct.phBudgetName")} /></Field>
        <Field label={t("acct.periodicity")}><Select value={periodicity} onChange={(e) => setPeriodicity(e.target.value)}><option value="annual">{t("acct.annual")}</option><option value="quarterly">{t("acct.quarterly")}</option><option value="monthly">{t("acct.monthly")}</option></Select></Field>
        <Field label={t("acct.fiscalYear")}><Select value={fiscalYearId} onChange={(e) => setFiscalYearId(e.target.value)}><option value="">{t("fin.noneOption")}</option>{fiscalYears.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}</Select></Field>
      </div>
      {periodized && <p className="text-[12px] text-ink-3">{t("acct.budgetPeriodNote")}</p>}
      <div className="overflow-x-auto rounded-lg border border-line">
        <table className="w-full min-w-[560px] text-[13px]">
          <thead><tr className="border-b border-line bg-surface-2 text-[11px] uppercase tracking-wide text-ink-3">
            ...
            {periodized && <th className="w-28 px-2 py-2 text-start font-medium">{t("acct.month")}</th>}
            <th className="w-40 px-2 py-2 text-end font-medium">{t("acct.budgetedAmount")}</th>
          ...
          <tbody>
            {lines.map((l, i) => (
              <tr key={i} className="border-b border-line last:border-0">
                ...
                {periodized && <td className="px-2 py-1.5"><Select value={l.month} onChange={(e) => setLine(i, { month: e.target.value })} className="h-8">{MONTHS.map((m, idx) => <option key={m} value={idx}>{t(`acct.${m}`)}</option>)}</Select></td>}
      ...
      <div className="flex items-center justify-between">
        <Button variant="ghost" size="sm" onClick={() => setLines((ls) => [...ls, { accountId: accounts[0]?.id ?? "", amount: "0", month: "0" }])}><Plus className="h-4 w-4" /> {t("fin.addLine")}</Button>
        <div className="text-[13px] font-semibold">{t("common.total")} <span className="ms-2 tabular text-ink">{fmt(total)}</span></div>
      </div>
      <div className="flex justify-end gap-2 border-t border-line pt-3">
        <Button variant="primary" size="sm" disabled={pending || !name || valid.length === 0} onClick={submit}>{budget ? t("acct.saveBudget") : t("acct.createBudget")}</Button>
      </div>
    </div>
  );
}
...
export function BudgetStatusButton({ id, status }: { id: string; status: string }) {
  ...
  return (
    <Button variant="secondary" size="sm" disabled={pending} onClick={() => start(async () => {
      ...
      if (r.ok) { toast({ kind: "success", title: next === "active" ? t("acct.budgetActivated") : t("acct.budgetDrafted") }); router.refresh(); } else toast({ kind: "error", title: r.error });
    })}>{next === "active" ? t("fin.activate") : t("acct.setDraft")}</Button>
  );
}
