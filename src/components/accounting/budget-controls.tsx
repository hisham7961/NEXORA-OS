...
export function BudgetComposer({ companyId, accounts, fiscalYears, budget }: { companyId: string; accounts: AccountOption[]; fiscalYears: FyOption[]; budget?: { id: string; name: string; fiscalYearId: string | null; currency: string; lines: { accountId: string; amount: string }[] } }) {
  const router = useRouter(); const { toast } = useToast();
  const [pending, start] = useTransition();
  const [name, setName] = useState(budget?.name ?? "");
  const [fiscalYearId, setFiscalYearId] = useState(budget?.fiscalYearId ?? fiscalYears[0]?.id ?? "");
  const [lines, setLines] = useState<LineDraft[]>(budget?.lines.map((l) => ({ accountId: l.accountId, amount: l.amount })) ?? [{ accountId: accounts[0]?.id ?? "", amount: "0" }]);
  ...
  const submit = () => start(async () => {
    const payload = { name, fiscalYearId: fiscalYearId || undefined, lines: valid.map((l) => ({ accountId: l.accountId, amount: Number(l.amount) || 0 })) };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <Button variant="ghost" size="sm" onClick={() => setLines((ls) => [...ls, { accountId: accounts[0]?.id ?? "", amount: "0" }])}><Plus className="h-4 w-4" /> Add line</Button>
