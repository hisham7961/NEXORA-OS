    ...
    <div className="space-y-2">
      {steps.map((id, i) => (
        <div key={id} className="flex items-center gap-2">
          ...
          <button type="button" onClick={() => setSteps((s) => s.filter((x) => x !== id))} className="text-ink-3 hover:text-critical" aria-label={t("naf.remove")}><X className="h-3.5 w-3.5" /></button>
        </div>
      ))}
      {available.length > 0 && (
        <Select value="" onChange={(e) => e.target.value && setSteps((s) => [...s, e.target.value])} aria-label={t("naf.addApproverAria")}>
          <option value="">{t("naf.addApproverStep")}</option>
          {available.map((u) => <option key={u.id} value={u.id}>{u.label}</option>)}
        </Select>
      )}
      {steps.length === 0 && <p className="text-xs text-ink-3">{t("naf.approversInOrder")}</p>}
    </div>
  );
}
...
export function NewApprovalButton({ options }: { options: { users: Option[]; brands: Option[]; companies: Option[] } }) {
  ...
  return (
    <>
      <Button variant="primary" onClick={() => setOpen(true)}><Plus className="h-4 w-4" /> {t("naf.newRequest")}</Button>
      <Drawer open={open} onClose={() => setOpen(false)} title={t("naf.submitApproval")} description={t("naf.submitApprovalSub")}>
        <ActionForm action={createApprovalAction} submitLabel={t("naf.submit")} onCancel={() => setOpen(false)} onSuccess={() => { setOpen(false); router.refresh(); }}>
          <FormField label={t("chat.title")} name="title" required>
            <Input name="title" placeholder={t("naf.phTitle")} required />
          </FormField>
          <FormSection>
            <FormField label={t("common.type")} name="type">
          ...
          <FormField label={t("camp.notes")} name="notes">
            <Textarea name="notes" placeholder={t("naf.phNotes")} />
          </FormField>
          <FormField label={t("naf.approvalChain")} name="approverIds" required>
