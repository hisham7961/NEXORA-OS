...
export function FavoriteStar({ entityType, entityId, label, href, initial }: { entityType: string; entityId: string; label?: string; href?: string; initial?: boolean }) {
  ...
  return (
    <button title={fav ? t("bm.removeFavorite") : t("bm.addFavorite")} disabled={pending} className={`inline-flex h-8 w-8 items-center justify-center rounded-lg border border-line ${fav ? "text-amber-500" : "text-ink-3 hover:text-ink"}`}
      onClick={() => start(async () => {
...
export function SavedViews({ module }: { module: string }) {
  const router = useRouter(); const pathname = usePathname(); const params = useSearchParams(); const { toast } = useToast(); const { t } = useI18n();
  ...
  const saveCurrent = () => start(async () => {
    const name = window.prompt(t("bm.namePrompt")); if (!name) return;
    const filtersJson = params.toString();
    const r = await saveViewAction({ module, name, filtersJson });
    if (r.ok) { toast({ kind: "success", title: t("bm.viewSaved") }); load(); } else toast({ kind: "error", title: r.error });
  });
  ...
  return (
    <div className="relative">
      <Button variant="ghost" size="sm" onClick={() => setOpen((o) => !o)}><BookmarkPlus className="h-4 w-4" /> {t("bm.views")} <ChevronDown className="h-3.5 w-3.5" /></Button>
      {open && (
        <div className="absolute end-0 z-20 mt-1 w-64 rounded-lg border border-line bg-surface p-1 shadow-lg">
          <button className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-[13px] text-ink hover:bg-surface-2" disabled={pending} onClick={saveCurrent}><BookmarkPlus className="h-3.5 w-3.5" /> {t("bm.saveCurrent")}</button>
          <div className="my-1 border-t border-line" />
          {views === null ? <div className="px-2 py-1.5 text-[12px] text-ink-3">{t("common.loading")}</div>
            : views.length === 0 ? <div className="px-2 py-1.5 text-[12px] text-ink-3">{t("bm.noViews")}</div>
            : views.map((v) => (
              <div key={v.id} className="group flex items-center justify-between gap-1 rounded px-2 py-1 hover:bg-surface-2">
                <button className="min-w-0 flex-1 truncate text-start text-[13px] text-ink-2" onClick={() => apply(v)}>{v.name}{v.isShared ? ` ·${t("bm.shared")}` : ""}</button>
