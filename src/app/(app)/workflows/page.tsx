...
export default async function WorkflowsPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  const columns: Column<WorkflowDefinition>[] = [
    ...
    { key: "status", header: t("common.status"), render: (w) => <Badge category={STATUS_CAT[w.status] ?? "neutral"}>{t(`status.${w.status}`)}</Badge> },
  ...
  return (
    <ResourceList title={t("workflows.title")} description={t("workflows.subtitle")}
      ...
      filters={[{ name: "status", label: t("common.status"), options: ["draft", "active", "archived"].map((v) => ({ value: v, label: t(`status.${v}`) })) }]}
      ...
      empty={<EmptyState icon={<Workflow className="h-5 w-5" />} title={t("workflows.empty")} description={t("wf.emptyBody")} />} />
  );
}
