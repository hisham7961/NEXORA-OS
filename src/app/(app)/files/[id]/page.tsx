...
export default async function FileDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <div className="mb-1 text-xs text-ink-3"><Link href="/files" className="hover:text-ink-2">Files</Link> / {file.name}</div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="mt-0.5 flex items-center gap-2 text-xs text-ink-3">
              ...
              {file.tags.map((t) => <Badge key={t} category="info">{t}</Badge>)}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <a href={fileDownloadHref(file.id)} className="inline-flex items-center gap-1.5 rounded-md bg-accent px-3 py-1.5 text-[13px] font-medium text-white hover:bg-accent/90"><Download className="h-4 w-4" /> Download</a>
          {isPreviewable(file.mimeType) && <a href={fileDownloadHref(file.id, { inline: true })} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-md border border-line px-3 py-1.5 text-[13px] text-ink-2 hover:bg-surface-2"><Eye className="h-4 w-4" /> Preview</a>}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          <Panel>
            <PanelHeader title="Versions" description="Append-only history — a new version never overwrites a previous one." />
            <ul className="divide-y divide-line">
              {versions.map((v) => (
                <li key={v.id} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <a href={fileDownloadHref(file.id, { versionId: v.id })} className="text-ink-3 hover:text-accent" aria-label={`Download v${v.version}`}><Download className="h-3.5 w-3.5" /></a>
                </li>
              ))}
            </ul>
          </Panel>
        ...
        <Panel>
          ...
          <PanelBody>
            <dl className="space-y-2.5 text-[13px]">
              <Row label="Owner">{file.ownerId ? <UserChip name={refName(lookups.users, file.ownerId)} color={lookups.users.get(file.ownerId)?.meta} /> : "—"}</Row>
              <Row label="Brand"><BrandChip name={refName(lookups.brands, file.brandId)} color={file.brandId ? lookups.brands.get(file.brandId)?.meta : null} /></Row>
              <Row label="Market"><CountryChip name={refName(lookups.countries, file.countryId)} iso2={file.countryId ? lookups.countries.get(file.countryId)?.meta : null} /></Row>
              <Row label="Visibility"><span className="capitalize text-ink">{file.visibility}</span></Row>
              <Row label="Type"><span className="text-ink-2">{file.mimeType ?? "—"}</span></Row>
              <Row label="Uploaded"><span className="text-ink-2">{formatDateTime(file.createdAt, locale)}</span></Row>
            </dl>
