...
export default async function ChartOfAccountsPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  const columns: Column<Row>[] = [
    ...
    { key: "name", header: t("acct.col.account"), render: (a) => <span className="text-ink">{a.name}{a.isSystem ? <Badge category="info" className="ms-2">{t("acct.systemBadge")}</Badge> : null}</span> },
    { key: "type", header: t("common.type"), render: (a) => <Badge category={TYPE_CAT[a.type] ?? "neutral"}>{t(`fin.acctType.${a.type}`)}</Badge> },
    { key: "normal", header: t("acct.normal"), render: (a) => <span className="text-ink-3 capitalize">{a.normalBalance}</span> },
    { key: "posting", header: t("acct.posting"), align: "center", render: (a) => (a.allowPosting ? <span className="text-success">✓</span> : <span className="text-ink-3">{t("acct.headerAcct")}</span>) },
    { key: "active", header: t("common.status"), align: "end", render: (a) => <Badge category={a.isActive ? "success" : "neutral"}>{a.isActive ? t("status.active") : t("status.inactive")}</Badge> },
  ];

  return (
    <>
      <PageHeader title={t("acct.chart")} description={t("acct.chartSub", { name: current.name })}
      ...
      <Panel>
        <DataTable columns={columns} rows={accounts} getRowKey={(a) => a.id}
          empty={<EmptyState icon={<BookOpenCheck className="h-5 w-5" />} title={t("acct.noAccounts")} description={t("acct.noAccountsBody")} />} />
      </Panel>
    </>
  );
}
