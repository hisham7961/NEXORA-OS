...
export default async function CreditNotesPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  const columns: Column<Row>[] = [
    { key: "number", header: t("acct.col.number"), render: (c) => <Link href={`/accounting/credit-notes/${c.id}`} className="font-mono text-ink hover:text-accent">{c.creditNoteNumber ?? "draft"}</Link> },
  ...
  return (
    <>
      <PageHeader title={t("acct.creditNotes")} description={`Customer credit notes for ${current.name} (${current.baseCurrency}).`}
