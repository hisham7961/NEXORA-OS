...
export default async function CreditNotesPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  if (!current) return <><PageHeader title="Credit Notes" /><Panel><EmptyState title="No company in scope" /></Panel></>;
  ...
  const columns: Column<Row>[] = [
    { key: "number", header: "Number", render: (c) => <Link href={`/accounting/credit-notes/${c.id}`} className="font-mono text-ink hover:text-accent">{c.creditNoteNumber ?? "draft"}</Link> },
    { key: "customer", header: "Customer", render: (c) => <span className="text-ink-2">{c.customer.name}</span> },
  ...
  return (
    <>
      <PageHeader title="Credit Notes" description={`Customer credit notes for ${current.name} (${current.baseCurrency}).`}
        actions={<div className="flex items-center gap-2"><CompanyPicker companies={companies} current={current.id} />{canCreate && <Link href={`/accounting/credit-notes/new?company=${current.id}`}><Button variant="primary" size="sm"><Plus className="h-4 w-4" /> New credit note</Button></Link>}</div>} />
      <Panel>
        <DataTable columns={columns} rows={rows} getRowKey={(c) => c.id}
          empty={<EmptyState icon={<FileMinus className="h-5 w-5" />} title="No credit notes" description="Issue a credit note to reduce a customer's receivable." />} />
      </Panel>
    </>
  );
}
