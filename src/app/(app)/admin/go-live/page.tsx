  ...
  return (
    <>
      <PageHeader
        ...
        description={`Everything ${current.name} needs before real employees operate it. Complete each step from its own screen — no database or scripts required.`}
        actions={<div className="flex items-center gap-2">
          <CompanyPicker companies={companies} current={current.id} />
          {report.ready ? <Badge category="success">Ready for go-live</Badge> : <Badge category="warning">{report.done}/{report.total} done</Badge>}
        </div>}
      />
      <Panel>
              ...
              <li key={it.key} className="flex items-center justify-between gap-3 px-4 py-3">
                <div className="flex min-w-0 items-start gap-2.5">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 text-[13px] font-medium text-ink">{it.label}{!it.required && <span className="rounded bg-surface-2 px-1.5 py-0.5 text-[10px] font-semibold uppercase text-ink-3">optional</span>}</div>
                ...
                {it.href && !it.done && (
                  <Link href={`${it.href}?company=${current.id}`} className="inline-flex shrink-0 items-center gap-1 rounded-md border border-line-strong px-2.5 py-1 text-[12px] text-accent hover:bg-surface-2">
                    Set up <ArrowRight className="h-3.5 w-3.5 flip-x" />
                  </Link>
                )}
              </li>
            ))}
      ...
      {report.ready && (
        <p className="mt-3 inline-flex items-center gap-1.5 text-[13px] text-success"><Rocket className="h-4 w-4" /> {current.name} is ready — employees, finance and admins can operate it now.</p>
      )}
    </>
  );
}
