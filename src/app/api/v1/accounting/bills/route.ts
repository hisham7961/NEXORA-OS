...
import { listBills, createBill } from "@/domain/accounting/ap";
...
export const GET = route(async ({ principal, req }) => {
  ...
  const res = await listBills(principal, companyId, Object.fromEntries(u.searchParams));
...
export const POST = route(async ({ principal, ip, userAgent, req }) => {
  ...
  if (!companyId) throw new ServiceError("missing_company", "companyId is required", 400);
  return ok(await createBill({ principal, ip, userAgent }, companyId, rest));
});
