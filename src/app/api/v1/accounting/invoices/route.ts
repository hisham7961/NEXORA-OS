...
import { listInvoices, createInvoice } from "@/domain/accounting/ar";
...
export const GET = route(async ({ principal, req }) => {
  ...
  const res = await listInvoices(principal, companyId, Object.fromEntries(u.searchParams));
...
export const POST = route(async ({ principal, ip, userAgent, req }) => {
  ...
  if (!companyId) throw new ServiceError("missing_company", "companyId is required", 400);
  return ok(await createInvoice({ principal, ip, userAgent }, companyId, rest));
});
