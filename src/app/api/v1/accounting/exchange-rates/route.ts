...
import { listExchangeRates, upsertExchangeRate } from "@/domain/accounting/setup";
export const GET = route(async ({ principal, req }) => {
  ...
  return ok(await listExchangeRates(principal, companyId));
});
export const POST = route(async ({ principal, ip, userAgent, req }) => {
  ...
  if (!companyId) throw new ServiceError("missing_company", "companyId is required", 400);
  return ok(await upsertExchangeRate({ principal, ip, userAgent }, companyId, rest));
});
