...
import { listReconciliations, createReconciliation } from "@/domain/accounting/bank";
...
export const GET = route(async ({ principal, req }) => {
  ...
  return ok(await listReconciliations(principal, companyId));
});
export const POST = route(async ({ principal, ip, userAgent, req }) => {
  ...
  if (!companyId) throw new ServiceError("missing_company", "companyId is required", 400);
  return ok(await createReconciliation({ principal, ip, userAgent }, companyId, rest));
});
