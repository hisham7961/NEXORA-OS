...
import { updateCompany, archiveCompany } from "@/domain/org-admin";
export const PATCH = route<{ id: string }>(async ({ principal, ip, userAgent, params, req }) => {
  ...
  return ok(await updateCompany({ principal, ip, userAgent }, params.id, body));
});
export const DELETE = route<{ id: string }>(async ({ principal, ip, userAgent, params }) => {
  await archiveCompany({ principal, ip, userAgent }, params.id);
