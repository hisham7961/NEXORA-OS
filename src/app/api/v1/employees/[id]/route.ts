...
import { updateEmployee, archiveEmployee } from "@/domain/org-admin";
export const PATCH = route<{ id: string }>(async ({ principal, ip, userAgent, params, req }) => {
  ...
  return ok(await updateEmployee({ principal, ip, userAgent }, params.id, body));
});
export const DELETE = route<{ id: string }>(async ({ principal, ip, userAgent, params }) => {
  await archiveEmployee({ principal, ip, userAgent }, params.id);
