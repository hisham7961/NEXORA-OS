...
import { setPublishingStatus } from "@/domain/social";

/** POST /api/v1/social/[id]/status — change lifecycle status (mobile/API parity §30). */
export const POST = route<{ id: string }>(async ({ principal, ip, userAgent, params, req }) => {
  ...
  return ok(await setPublishingStatus({ principal, ip, userAgent }, params.id, status));
});
