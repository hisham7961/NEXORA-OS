...
import { setWhatsappStatus } from "@/domain/whatsapp";

/** POST /api/v1/whatsapp/[id]/status — change lifecycle status (mobile/API parity §30). */
export const POST = route<{ id: string }>(async ({ principal, ip, userAgent, params, req }) => {
  ...
  return ok(await setWhatsappStatus({ principal, ip, userAgent }, params.id, status));
});
