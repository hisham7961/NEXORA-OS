...
import { listTeams, teamQuerySchema } from "@/domain/teams";
import { createTeam } from "@/domain/org-admin";

export const GET = route(async ({ principal, req }) => {
  const query = teamQuerySchema.parse(Object.fromEntries(new URL(req.url).searchParams));
...
export const POST = route(async ({ principal, ip, userAgent, req }) => {
  ...
  return ok(await createTeam({ principal, ip, userAgent }, body));
});
