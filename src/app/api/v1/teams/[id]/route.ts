...
import { updateTeam, archiveTeam } from "@/domain/org-admin";
export const PATCH = route<{ id: string }>(async ({ principal, ip, userAgent, params, req }) => {
  ...
  return ok(await updateTeam({ principal, ip, userAgent }, params.id, body));
});
export const DELETE = route<{ id: string }>(async ({ principal, ip, userAgent, params }) => {
  await archiveTeam({ principal, ip, userAgent }, params.id);
